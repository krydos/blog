...
.PHONY: debug
debug:
	docker run --rm -it -v $(CURDIR):/blog -p 4000:4000 -w="/blog" $(RUBY_CONTAINER_NAME) bash 


.PHONY: up
up:
	docker run --rm -it -v $(CURDIR):/blog -p 4000:4000 -w="/blog" $(RUBY_CONTAINER_NAME) bash -c 'bundle config --local path ./vendor; bundle exec jekyll serve --host=0.0.0.0'
